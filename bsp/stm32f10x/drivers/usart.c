...
static rt_size_t stm32_dma_transmit(struct rt_serial_device *serial, rt_uint8_t *buf, rt_size_t size, int direction)
{
    ...
    switch(direction)
    {
        ...
        case RT_SERIAL_DMA_TX:
            ...
            DMA_InitStructure.DMA_Priority = DMA_Priority_High;
            DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;
            DMA_Init(uart->dma.tx_ch, &DMA_InitStructure);
//            DMA_ClearFlag(uart->dma.tx_gl_flag);
//            DMA_ITConfig(uart->dma.tx_ch, DMA_IT_TC, ENABLE);
            USART_DMACmd(uart->uart_device, USART_DMAReq_Tx, ENABLE);
            DMA_Cmd(uart->dma.tx_ch, ENABLE);
...
/**
 * DMA send done process. This need add to DMA send done ISR.
 *
 * @param serial serial device
 */
...
//    send_len = uart->dma.setting_recv_len - uart->dma.last_recv_index;
//    /* reset last recv index */
//    uart->dma.last_recv_index = 0;
    /* enable interrupt */
    rt_hw_interrupt_enable(level);
...
void USART2_IRQHandler(void)
{
    ...
    rt_interrupt_enter();
     ...
     if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
     {
         ...
         {
             uart_rx_check_data(&uart2_rx, (USART2->DR & 0xff));
         }
         /* clear interrupt */
...
void USART3_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)
    {
        ...
        {
            uart_rx_check_data(&uart3_rx, (USART3->DR & 0xff));
        }
        /* clear interrupt */
...
void UART4_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)
    {
        ...
        {
            uart_rx_check_data(&uart4_rx, (UART4->DR & 0xff));
        }
        /* clear interrupt */
...
void UART5_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(UART5, USART_IT_RXNE) != RESET)
    {
        ...
        {
            uart_rx_check_data(&uart5_rx, (UART5->DR & 0xff));
        }
        /* clear interrupt */
...
static void DMA_Configuration(struct rt_serial_device *serial) {
    ...
    if (serial->parent.open_flag & RT_DEVICE_FLAG_DMA_TX)
    {
        ...
        DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;
        DMA_InitStructure.DMA_BufferSize = 0;//serial->config.bufsz;
        DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
        DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
        DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
        DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
